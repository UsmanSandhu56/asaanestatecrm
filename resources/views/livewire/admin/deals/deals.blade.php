...
<div class="ecommerce-application auto-match-page close-d-deal auto-match-open-page">
    ...
    <div class="content-wrapper container-xxl p-0">
        <div class="content-header row">
            <div class="content-header-left col-md-12 col-sm-12 col-12 mb-1">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        <h2 class="content-header-title float-start mb-2">{{__('Close The Deal')}}</h2>
                        <div class="breadcrumb-wrapper">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="{{route('dashboard')}}">{{__('Dashboard')}}</a>
                                </li>
                                <li class="breadcrumb-item active">{{__('Close The Deal')}}
                                </li>
                            </ol>
                        </div>
                    </div>
        ...
        <div class="content-body row">
                ...
                <div class="match-amenities">
                    <div class="position-relative ">
                        <div class="input-group input-group-merge mb-1 shadow">
                            <input wire:model.debounce.500ms="propertySearch"
                                   x-ref="propertySearch"
                                   @keydown.window="if (event.keyCode === 191) {event.preventDefault(); $refs.propertySearch.focus();}"
                                   @focus="isOpen = true" @keydown="isOpen = true"
                                   @keydown.escape.window="isOpen = false"
                                   @keydown.shift.tab="isOpen = false"
                                   type="number" id="phone_no"
                        ...
                        @if (strlen($propertySearch) >= 5)
                            <div class="rounded w-100 mt-50 position-absolute serch-field-result"
                                 x-show.transition.opacity="isOpen">
                                @if ($propertySearchResults->count() > 0)
                                    <ul class="list-group">
                                        @foreach ($propertySearchResults->take(5) as $result)
                                            <li class="list-group-item p-1 ">

                                                <div class="">
                                                    @foreach($result->properties as $pro)
                                                        <div
                                                            class="d-block w-100 @if(!$loop->last) serch-field-list @endif">
                                                            <a wire:click.prevent="setProperty({{ $pro }})"
                                                               @click="isOpen = false"
                                                               class="block border-0 bg-transparent fw-bolder w-100"
                                                               @if ($loop->last) @keydown.tab="isOpen = false" @endif>
                                                                <div class="d-flex align-items-center">
                                                                    <p class="mb-0">{{$pro->title }}</p>
                                                                    <p class="ms-1 mb-0 badge bg-primary">{{($pro->purpose === 0) ? 'Sell' : 'Rent'  }}</p>
                                                                </div>
                                                                <div class="fw-bold fs-6 d-flex serch-field-det">
                                                                    <p class="mb-0 me-2"
                                                                       @click="isOpen = false">{{ $result->name }}</p>
                                                                    <p class="mb-0 me-2">{{ $pro->price}} PKR</p>
                                                                </div>
                                                            </a>
                                                        </div>
                                                    @endforeach
            ...
            <div class="col-md-6 col-6 content-right-det">
                ...
                <div class=" match-amenities" x-data="{ isOpen: true }" @click.away="isOpen = false">
                    <div class="position-relative ">
                        <div class="input-group input-group-merge mb-1 shadow">
                            <input wire:model.debounce.500ms="requirementSearch"
                                   x-ref="requirementSearch"
                        ...
                        @if (strlen($requirementSearch) >= 5)
                            <div class="rounded w-100 mt-50 position-absolute serch-field-result"
                                 x-show.transition.opacity="isOpen">
                                @if ($requirementSearchResults->count() > 0)
                                    <ul class="list-group">
                                        @foreach ($requirementSearchResults->take(5) as $result)
                                            <li class="list-group-item p-1 ">
                                                ...
                                                <div class="">
                                                    @foreach($result->propertyRequirements as $requirement)
                                                        <div
                                                            class="d-block w-100 @if(!$loop->last) serch-field-list @endif">
                                                            <a wire:click.prevent="setPropertyRequirement({{ $requirement }})"
                                                               @click="isOpen = false"
                                                               class="block border-0 bg-transparent fw-bolder w-100"
                                                               @if ($loop->last) @keydown.tab="isOpen = false" @endif>
                                                                <div class="d-flex align-items-center">
                                                                    <p class="mb-0">{{$requirement->title }}</p>
                                                                    <p class="ms-1 mb-0 badge bg-primary">{{($requirement->purpose === 0) ? 'Buy' : 'Rent'  }}</p>
                                                                </div>
                                                                <div class="fw-bold fs-6 d-flex serch-field-det">
                                                                    <p class="mb-0 me-2"
                                                                       @click="isOpen = false">{{ $result->name }}</p>
                                                                    <p class="mb-0 me-2">{{ $requirement->min_price .' - '. $requirement->max_price}}
                                                                        PKR</p>
                                                                </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 col-12">
                                <div class="match-facing">
                                    <h4>{{__('Budget')}}: <span
                                            class="fs-5">{{$propertyRequirement->min_price}} - {{$propertyRequirement->max_price}}</span>
                                    </h4>
                                    <h4>{{__('Area')}}: <span
                                            class="fs-5">{{$propertyRequirement->min_area}} - {{$propertyRequirement->max_area}}</span>
                                    </h4>
                                </div>
                            </div>
                        </div>
            ...
            <div class="col-md-6 col-6">
                @isset($property)
                    <div class="card-body cntct-card-det pe-5">
                        <h3 class="mb-75">{{($property->purpose === 0) ? 'Seller' : 'Owner'}}</h3>
                        ...
                        <!-- <div class="prof-img">
                            ...
                            <div class="prof-img-det">
                                ...
                                <a href="#" class="card-text"><i
                                        class="fa fa-envelope"></i> {{$property->customer->email}}
                                </a><br>
                                <a href="#" class="card-text"><i
                                        class="fa fa-map-marker"></i> {{$property->address->location}}
                                </a>
                                <br>
                                <a href="#" class="card-text"><i
                                        class="fa fa-phone"></i> {{$property->customer->phone_no}}
                                </a>
                            </div>
                        </div>
            ...
            <div class="col-md-6 col-6 content-right-det">
                @isset($propertyRequirement)
                    <div class="card-body cntct-card-det pe-5 cntct-card-buyer">
                        <h3 class="mb-75">{{($propertyRequirement->purpose === 0) ? 'Buyer' : 'Tenant'}}</h3>
                        <div class="mt-1 prof-img-set">
                        <!-- <div class="prof-img">
                                <img src="{{asset('app-assets/images/avatars/12-small.png')}}">
                            </div> -->
                            <div class="prof-img-det">
                                ...
                                <a href="#" class="card-text"><i
                                        class="fa fa-map-marker"></i> {{$propertyRequirement->propertyRequirementDetail->location}}
                                </a>
                                <br>
                                <a href="#" class="card-text"><i
                                        class="fa fa-phone"></i> {{$propertyRequirement->customer->phone_no}}
                                </a>
                            </div>
                        </div>
                        <div class="am-cntct-phn">
                            <a href="tel:{{$propertyRequirement->customer->phone_no}}"><i
        ...
        @isset($property , $propertyRequirement)
            <div class="row">
                <div class="col-12 mt-3 mb-3">
                    <h2 class="content-header-title mb-2 border-0"><i
                            class="fa fa-star"></i> {{__('Deal Details (Rent/Sale)')}}
                    </h2>
                    <section id="basic-vertical-layouts">
                        <div class="row">
                            <div class="col-12">
                                <form wire:submit.prevent="store" class="form form-vertical">
                                    @error('propertyRequirement', 'property')
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-6 col-12">
                                            <div class="mb-1">
                                                ...
                                                <input wire:model="amount" type="number" id="amount" step="any"
                                        ...
                                        <div class="col-sm-6 col-12">
                                            <div class="mb-1">
                                                <label class="form-label" for="agency_commission">{{__('Agency Commission')}}</label>
                                                <input type="number" wire:model.debounce.100ms="commission"
                                                       wire:model="agency_commission"
                                                       step="any"
                                                       id="agency_commission"
                                                       class="form-control @error('agency_commission') is-invalid @enderror"
                                                       name="agency_commission" placeholder="{{__('Agency Commission')}}"/>
                                        ...
                                        <div class="col-sm-6 col-12">
                                            <div class="mb-1">
                                                <label class="form-label" for="agent_commission">{{__('Agent Commission')}}</label>
                                                <input type="number" wire:model="agent_commission"
                                                       id="agent_commission" step="any"
                                        ...
                                        <div class="col-12 col-sm-6 col-md-4 mx-auto text-center">
                                            <button type="submit" class="btn btn-warning fw-bolder fs-2 w-100">{{__('Deal Close')}}
                                            </button>
                                        </div>
                                    </div>
                                </form>
